...
- **备份配置**：一键下载配置文件备份
- **可视化配置**：内置「猫猫配置可视化编辑器 v2.0」，按区域表单化编辑配置（详见下方「可视化配置编辑器」章节）
...
点击插件按钮栏中的「🎛️ 可视化配置」即可打开内置的配置可视化编辑器（v2.0），无需手动编辑 YAML 文本即可完成大部分配置调整。

### 功能区域

| 区域 | 说明 |
|------|------|
| 基础设置 | 混合代理端口、代理模式、日志级别、面板密码、TProxy 端口等常用字段 |
| DNS 配置 | DNS 开关、增强模式、各级 nameserver 列表的增删改 |
| 订阅链接 | `proxy-providers` 的类型 / URL / 路径 / 更新间隔管理 |
| 规则集 | `rule-providers` 的行为 / 格式 / 类型 / URL 等管理 |
| 代理规则 | 逐条编辑、上下移动排序、删除 `rules` 列表 |
| 自定义规则 | 按域名 / IP / 关键词三种方式快速添加规则，支持从代理组列表选择策略 |
| 代理组 | 名称 / 类型 / 过滤关键词 / 成员节点编辑，支持上下排序 |
| 原始编辑 | YAML 解析失败时的手动修复模式，修复后可一键恢复可视化 |

### 操作说明

1. 首次打开点击「📂 读取当前配置」加载设备上的 `config.yaml`
2. 也可「📥 导入配置文件」在本地编辑，或「💾 导出配置」下载当前配置
3. 「✅ 验证配置」可在保存前检查 YAML 结构合法性
4. 「💾 保存并重启」保存前自动备份原配置到 uploads 目录，并通过 `Clash.Service` 重启内核
5. 若解析失败会自动进入「原始编辑」模式，手动修复后点击「🔄 重新解析」恢复可视化

### 技术说明

- 编辑器使用内置的轻量 YAML 解析器（`parseYAML` / `stringifyYAML`），无外部依赖
- 自定义规则以标记注释块（`猫猫配置编辑器-自定义规则开始/结束`）内嵌在 `config.yaml` 的 `rules` 列表中，保存时自动重建
- 重启逻辑与主插件一致，均走 `Clash.Service stop/start`，保证规则合并脚本正常执行
...
5. **高级配置**：点击「🎛️ 可视化配置」，在可视化编辑器中按区域调整 DNS / 规则集 / 代理组等配置
6. **查看面板**：点击「打开面板」，在浏览器中管理代理
...
6. 建议定期备份配置文件
7. 可视化编辑器保存时会自动备份原配置到 uploads 目录（`mm_config_backup_*.yaml`），如配置异常可从备份恢复
8. 可视化编辑器与「订阅刷新」共用同一份 `config.yaml`：整份刷新订阅后，编辑器内的修改会被覆盖，请重新读取配置后再编辑